...
class Quadruped(object):
  ...
  def SetRefVelocities(self, ref_velocities=[1.0,0.0]):
    self.ref_velocities = ref_velocities
  ...
  def GetBaseRefVelocities(self):
    return self.ref_velocities
  ...
  def GetFeetVelocities(self):
      motor_velocities = self.GetMotorVelocities()
      nbr_leg = 4
      v_leg = []
      for i in range(nbr_leg):
          J, _ = self.ComputeJacobianAndPosition(i)
          # Get current foot velocity in leg frame (Equation 2)
          v_leg.append(J @ motor_velocities[i * 3:(i * 3 + 3)])

      v = [item for sublist in v_leg for item in sublist]
  ...
  def GetFeetPosition(self):
    ...
    nbr_leg = 4
    for i in range(nbr_leg):
      _, pos = self.ComputeJacobianAndPosition(i)
      feet_position.append(pos)

    position = [item for sublist in feet_position for item in sublist]
    return position
